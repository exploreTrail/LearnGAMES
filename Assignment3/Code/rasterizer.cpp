...
void rst::rasterizer::rasterize_triangle(
  ...
  int boundingBox[4];
  boundingBox[0] =
      std::floor(std::min(std::min(t.v[0][0], t.v[1][0]), t.v[2][0]));
  boundingBox[1] =
      std::floor(std::min(std::min(t.v[0][1], t.v[1][1]), t.v[2][1]));
  boundingBox[2] =
      std::ceil(std::max(std::max(t.v[0][0], t.v[1][0]), t.v[2][0]));
  boundingBox[3] =
      std::ceil(std::max(std::max(t.v[0][1], t.v[1][1]), t.v[2][1]));
      ...
      for (int j = boundingBox[1]; j <= boundingBox[3]; ++j) {
        ...
        for (int k = 0; k < 4; k++) {
          if (insideTriangle(float(i) + pos[k][0], float(j) + pos[k][1], t.v)) {
            auto [alpha, beta, gamma] = computeBarycentric2D(
                float(i) + pos[k][0], float(j) + pos[k][1], t.v);
            float w_reciprocal =
        ...
        if (num >= 2) {
          auto [alpha, beta, gamma] =
              computeBarycentric2D(float(i) + 0.5, float(j) + 0.5, t.v);
          Eigen::Vector3f interpolated_color =
              interpolate(alpha, beta, gamma, t.getColor(0), t.getColor(1),
                          t.getColor(2), 1);
          Eigen::Vector3f interpolated_normal =
              interpolate(alpha, beta, gamma, t.getNormal(0), t.getNormal(1),
                          t.getNormal(2), 1);
          Eigen::Vector2f interpolated_texcoords =
              interpolate(alpha, beta, gamma, t.getTexCoord(0),
                          t.getTexCoord(1), t.getTexCoord(2), 1);
      ...
      for (int j = boundingBox[1]; j <= boundingBox[3]; j++) {
        if (insideTriangle(float(i) + 0.5, float(j) + 0.5, t.v)) {
          ...
          if (z_interpolated < get_depth(i, j, 0)) {
            ...
            // triangle's color directly to the frame buffer, pass the color to
            // the shaders first to get the final color; Use: auto pixel_color =
...
void rst::rasterizer::set_pixel(float x, float y,
                                const Eigen::Vector3f &color) {
  auto index = (height - 1 - y) * width + (x);
...
void rst::rasterizer::set_depth(int x, int y, float depth, int num) {
  assert(num >= 0 && num <= 3);
  auto index = y * height * AA * AA + x * AA * AA + num;
